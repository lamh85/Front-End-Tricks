...
myController = myApp.controller("myController", ['$scope', function($scope){
    ...
    $scope.ipsums = [
        ...
        "Missing until dinner time rub face on owner pelt around the house and up and down stairs chasing phantoms, for intrigued by the shower, or hide head under blanket so no one can see and stretch, so lick arm hair. Damn that dog . Get video posted to internet for chasing red dot kitty power! for kitty loves pigs yet jump off balcony, onto stranger's head but stare out the window. Stretch swat at dog instantly break out into full speed gallop across the house for no reason or chew foot, nap all day spit up on light gray carpet instead of adjacent linoleum.",
    ...
    $scope.rowNumbers = function(contentArray){
        numOfRows = Math.ceil(contentArray/$scope.colsPerRow);
        array = [];
